...
####### Swapchain ###############################
    ...
    def createGraphicsPipeline(self):
        ...
        vertexShaderModule = self.createShaderModule("shaders/vert2.spv")
    ...
    def createVertexBuffer(self):
        ...
        self.vertexBuffer = vkCreateBuffer(self.device, bufferInfo, None)

        memRequirements = vkGetBufferMemoryRequirements(self.device, self.vertexBuffer)
        ...
        self.vertexBufferMemory = vkAllocateMemory(self.device, allocInfo, None)
        vkBindBufferMemory(self.device, self.vertexBuffer, self.vertexBufferMemory, 0)

        data = vkMapMemory(self.device, self.vertexBufferMemory, 0, bufferInfo.size, 0)
        ffi.memmove(data, self.vertices, bufferInfo.size)
