...
void VulkanEngine::loadModels() {
	...
	createVertexBuffer(statue);
	createIndexBuffer(statue);
	mainDeletionQueue.push_function([=]() {
		cleanupMesh(statue);
	});

	ground.loadModel("models/ground.obj");
	createVertexBuffer(ground);
	createIndexBuffer(ground);
	mainDeletionQueue.push_function([=]() {
		cleanupMesh(ground);
	});
}
...
void VulkanEngine::createVertexBuffer(Mesh& mesh) {
	...
	uploadToMemoryLocation(stagingBuffer.memory, mesh.vertices.data(), size);
...
void VulkanEngine::createIndexBuffer(Mesh& mesh) {
	...
	uploadToMemoryLocation(stagingBuffer.memory, mesh.indices.data(), size);
...
void VulkanEngine::drawFrame(Scene* scene) {
	...
	//bind ground mesh
	offsets[0] = 0;
	vkCmdBindVertexBuffers(cmd, 0, 1, &ground.vertexBuffer.buffer, offsets);
	vkCmdBindIndexBuffer(cmd, ground.indexBuffer.buffer, 0, VK_INDEX_TYPE_UINT32);

	//draw statues
	for (Ground* ground_ : scene->groundPieces) {
		//set up push constants
		glm::mat4 model = glm::translate(glm::mat4{ 1.0f }, ground_->position);
		...
		//draw
		vkCmdDrawIndexed(cmd, static_cast<uint32_t>(ground.indices.size()), 1, 0, 0, 0);
	}
